class DateTimeInput < SimpleForm::Inputs::DateTimeInput
  def input
    ...
    if timestamp.blank?
      ...
      timestamp = method_value if method_value # default value
    end

    #create the input

    datetime_input = @builder.text_field(attribute_name, { :data => { :format => 'MM/dd/yyyy HH:mm:ss' }, :value => timestamp }.merge(input_html_options))
    ...
    template.content_tag(:div, datetime_input + calendar_button, :data => { :behaviour => 'datetime_picker' }, :class => "input-append date", :id => attribute_name )
  end
end
